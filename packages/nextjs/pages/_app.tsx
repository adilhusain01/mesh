...
const ScaffoldEthApp = ({ Component, pageProps }: AppProps) => {
  ...
  const data = useScaffoldContractRead({ contractName: "MeshFactory", functionName: "getDataSetNFTs" });

  function getMeshFactoryContract(chainId: any) {
    return deployedContracts[chainId as keyof typeof deployedContracts].MeshFactory;
  }

  useEffect(() => {
    const getDatasets = async () => {
      console.log("targetNetwork", targetNetwork.name, targetNetwork.id);
      const result = getMeshFactoryContract(targetNetwork.id);
      console.log("MeshFactory Contract", result);
